...
def dataset_hijack(dataset, n_splits, split_idx):
    """
    Hijack the dataset to only return a subset of the data.
    """
    dataset.n_splits = n_splits
    dataset.split_idx = split_idx
    image_data = dataset.image_data
    sorted_keys = sorted(image_data.keys())
    logger.info(f"Splitting dataset into {n_splits} parts, using part {split_idx}")
    split_keys = array_split(sorted_keys, n_splits)
    dataset.image_data = {k: image_data[k] for k in split_keys[split_idx]}
    logger.info(f"Dataset hijacked. New length: {len(dataset)}")
...
def train(args):
    ...
    if args.n_splits is not None:
        assert args.split_idx is not None, "split_idx must be specified when n_splits is specified"
        assert args.n_splits > 1, f"n_splits must be greater than 1, but got {args.n_splits}"
        assert args.split_idx < args.n_splits, f"split_idx must be less than n_splits, but got {args.split_idx} and {args.n_splits}"
        os.environ["SPLIT_DATASET"] = str(args.n_splits)
        os.environ["SPLIT_DATASET_IDX"] = str(args.split_idx)
        logger.info(f"Splitting dataset into {args.n_splits} parts, using part {args.split_idx}")
    # データセットを準備する
    ...
    if args.cache_text_encoder_outputs:
        ...
        clean_memory_on_device(accelerator.device)
    if args.cache_only:
        logger.info("cache_only is enabled. Exiting.")
        return
...
def setup_parser() -> argparse.ArgumentParser:
    ...
    # cache-only arguments
    parser.add_argument(
        "--cache_latents_only",
        action="store_true",
        help="only cache latents (and text encoder outputs)/ latentのみをキャッシュする",
    )
    parser.add_argument(
        "--n_splits",
        type=int,
        default=None,
        help="Splits the dataset into n_splits parts and caches latents for each part.",
    )
    parser.add_argument(
        "--split_idx",
        type=int,
        default=0,
        help="The index of the split to cache latents for.",
    )
    return parser
